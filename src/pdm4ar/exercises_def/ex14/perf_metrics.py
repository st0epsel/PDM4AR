...
def ex14_metrics(sim_context: SimContext) -> Tuple[AllPlayerMetrics, List[PlayerMetrics]]:
    ...
    for player_name, agent_log in sim_context.log.items():
        ...
        if GlobalConfig.PLAYER_PERF_METRIC:
            print(f'player "{player_name}" scores: ')
            print(f"   # Goals delivered        = +{int(num_goal_delivered*100)}")
            print(f"   Collided                 = {int(has_collided*-500)}")
            print(f"   Travelled distance       = {(-0.5*dist):.2f}")
            print(f"   Actuation effort         = {(-actuation_effort*0.01):.2f}")
            print(f"   Compute time             = {(-max(0.0, avg_comp_time-0.1)*100):.2f}")
    ...
    if GlobalConfig.ALL_PLAYERS_PERF_METRIC:
        print(f"Total Score {all_player_metrics.reduce_to_score():.2f}: ")
        print(f"   Total goals delivered    = +{int(num_goals_delivered*100)}")
        print(f"   Total collision          = {int(num_collided_players*-500)}")
        print(f"   task accomplishment time = +{((max_sim_time - task_accomplishment_time) * 10):.2f}")
        print(f"   Total distance           = {(-0.5*total_travelled_distance):.2f}")
        print(f"   Total actuation effort   = {(-total_actuation_effort*0.01):.2f}")
        print(f"   Total compute time       = {(-max(0.0, avg_computation_time-0.1)*100):.2f}")
        print(f"   Global planning time     = {(-max(0.0, global_planning_time-60)*10):.2f}")
